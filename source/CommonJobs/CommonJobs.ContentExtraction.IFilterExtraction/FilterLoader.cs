...
namespace CommonJobs.ContentExtraction.IFilterExtraction
{
    ...
    internal static class FilterLoader
    {
        ...
        static Guid IFilterGUID = new Guid("89BCB740-6119-101A-BCB7-00DD010655AF");
        ...
        private static bool GetFilterDllAndClass(string ext, out string dllName, out string filterPersistClass)
        {
            if (!GetFilterDllAndClassFromCache(ext, out dllName, out filterPersistClass))
            {
                ...
                {
                    GetFilterDllAndClassFromPersistentHandler(persistentHandlerClass, out dllName, out filterPersistClass);
                }
        ...
        private static bool GetFilterDllAndClassFromPersistentHandler(string persistentHandlerClass, out string dllName, out string filterPersistClass)
        {
            ...
            string path = string.Format("Software\\Classes\\CLSID\\{0}\\PersistentAddinsRegistered\\{1}", persistentHandlerClass, "{" + IFilterGUID + "}");
            //if (Is64Bit) path = string.Format("Software\\Wow6432Node\\Classes\\CLSID\\{0}\\PersistentAddinsRegistered\\{1}", persistentHandlerClass, "{" + IFilterGUID + "}");

            filterPersistClass = ReadStrFromHKLM(path);
            if (String.IsNullOrEmpty(filterPersistClass))
                return false;

            //Read the dll name 

            path = @"Software\Classes\CLSID\" + filterPersistClass + @"\InprocServer32";
            //if (Is64Bit) path = @"Software\Wow6432Node\Classes\CLSID\" + filterPersistClass + @"\InprocServer32";

            dllName = ReadStrFromHKLM(path);
        ...
        private static string GetPersistentHandlerClassFromContentType(string ext)
        {
            ...
            //if (Is64Bit) path = @"Software\Wow6432Node\Classes\" + ext;
            
            string contentType = ReadStrFromHKLM(path,"Content Type");
            if (String.IsNullOrEmpty(contentType))
                return null;

            path = @"Software\Classes\MIME\Database\Content Type\" + contentType;
            //if (Is64Bit) path = @"Software\Wow6432Node\Classes\MIME\Database\Content Type\" + contentType;
            

            
            string contentTypeExtension = ReadStrFromHKLM(path, "Extension");
        ...
        {
            string path = @"Software\Classes\" + ext;
            //if (Is64Bit) path = @"Software\Classes\" + ext;

            //Get the DocumentType of this file extension
            string docType = ReadStrFromHKLM(path);
            if (String.IsNullOrEmpty(docType))
                return null;

            //Get the Class ID for this document type
            path = @"Software\Classes\" + docType + @"\CLSID";
            //if (Is64Bit) path = @"Software\Wow6432Node\Classes\" + docType + @"\CLSID";

            string docClass = ReadStrFromHKLM(path);
            if (String.IsNullOrEmpty(docClass))
                return null;

            //Now get the PersistentHandler for that Class ID
            path = @"Software\Classes\CLSID\" + docClass + @"\PersistentHandler";
            //if (Is64Bit) path = @"Software\Wow6432Node\Classes\CLSID\" + docClass + @"\PersistentHandler";

            
            return ReadStrFromHKLM(path);
        }
